...
CubeUniformBuffer* Camera_old::uniforms = 0;
GLFWwindow* Camera_old::window = 0;
Vec3 Camera_old::pos = Vec3( 0, 5.1, 0 );
Vec3 Camera_old::renderPos = Vec3( 0, 0, 0 );
Vec3 Camera_old::rot = Vec3( 0, 0, 0 );
double Camera_old::prev_x = 0;
double Camera_old::prev_y = 0;
float Camera_old::yVel = 0;
std::thread* Camera_old::cameraThread = 0;
Vec3* Camera_old::poss = 0;
int Camera_old::possSize = 0;
int Camera_old::fence = 0;
bool Camera_old::grounded = false;
VLKComputeContext* Camera_old::computeContext = NULL;
...
static void cameraThreadRun() {
	...
	while (!glfwWindowShouldClose(win)) {
		...
		for (int i = 0; i < Chunk::rcubesSize; i++) {
			...
			if (Camera_old::pos.dist(cube.pos) <= 8) {
		...
		if (cbsz > 0) {
			float tx = Camera_old::rot.x;
			float ty = Camera_old::rot.y;
			...
			posData[0] = Camera_old::pos.x;
			posData[1] = Camera_old::pos.y + 0.5f;
			posData[2] = Camera_old::pos.z;
			posData[3] = nx;
			posData[4] = ny;
			posData[5] = nz;
		...
		while (Camera_old::fence > 0) {
			std::this_thread::sleep_for(std::chrono::microseconds(10));
		}

		Camera_old::fence = 1;

		Camera_old::poss = vecs;
		Camera_old::possSize = cbsz;
...
void Camera_old::init(GLFWwindow* win, CubeUniformBuffer* puniforms, VLKContext* context) {
	window = win;
...
bool hittingCubes(Vec3 pos) {
	for (int i = 0; i < Camera_old::possSize; i++) {
		if (hittingCube(pos, Camera_old::poss[i])) {
			return true;
		}
	}
...
void Camera_old::update(float dt) {
...
void Camera_old::destroy(VLKDevice* device) {
	cameraThread->join();
